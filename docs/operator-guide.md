...
2. Register the client's intake: business type, target public function,
   prior filing history in Belgium if any.
3. Run the advisor in read-only mode against the federal e-Procurement platform (publicprocurement.be / BOSA) — e-Notification, e-Tendering, Free Market and e-Catalogue.
4. Compare the checklist against the client's current documentation
   (CBE (Crossroads Bank for Enterprises / Banque-Carrefour des Entreprises) enterprise number plus VAT registration).
...
- named referral relationship with Belgian-licensed counsel or a
...
a working referral relationship with Belgian-licensed counsel or a
registered agent for whatever licensed representation the law of
Belgium requires for actual public-procurement filings.
